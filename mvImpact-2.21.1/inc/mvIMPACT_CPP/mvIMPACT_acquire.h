...
#if defined(_MSC_VER) && (_MSC_VER >= 1400) // is at least VC 2005 compiler?
            errno_t result = strcpy_s( p->pChannels[i].szChannelDesc, DEFAULT_STRING_SIZE_LIMIT, channelDescriptors[i].c_str() );
            result = result; // remove release build warning
            assert( result == 0 );
#else
            strcpy( p->pChannels[i].szChannelDesc, channelDescriptors[i].c_str() );
#endif // #if defined(_MSC_VER) && (_MSC_VER >= 1400)
        }
...
public:
    ...
    /**
     *  This will \b NOT be a valid mvIMPACT acquire pixel format but e.g. a GigE Vision pixel format or something else.
     *  \note
     *  For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
     */
